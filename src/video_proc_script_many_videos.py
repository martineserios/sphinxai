...
videos_path =[file for file in os.listdir('../media_tmp')]
...
def vidoe_proc(video_path):
    ...
    cap = cv2.VideoCapture(f'../media_tmp/{video_path}')
    ...
    while True:
        ...
        if ret:
            ...
            # cv2.putText(frame, f'ROLL: {str(round(roll, 2))}', (90, 1700), cv2.FONT_HERSHEY_DUPLEX, 1.5, (100, 50, 150), 3)
                ...
                meta = test_meta(
                    test_id=test_id,
                    video_file_name=f'{video_name}_out',
                    datetime=dt_string,
                    fps=fps,
                    athlete=ATHLETE,
